...
app.mount(
    "/dist",
    StaticFiles(directory=os.path.join(os.getcwd(), "app", "templates", "dist")),
    name="dist",
)
app.mount(
    "/plugins",
    StaticFiles(directory=os.path.join(os.getcwd(), "app", "templates", "plugins")),
    name="plugins",
)
        ...
        try:
            ...
            if permission == "user":
                ...
                return templates.TemplateResponse(
                    ...
                    context={
                        "request": request,
                        "dashboard_tongsinhvien": tong_sinh_vien,
                        "dashboard_tiledadanhgia": ti_le_da_danh_gia,
                        "dashboard_soluongdat": so_luong_ket_qua["dat"],
                        "dashboard_soluongkhongdat": so_luong_ket_qua["khong_co_nhom"],
                    },
                )
            else:
...
async def hosonguoihuongdan(request: Request, id: str):
    ...
    return templates.TemplateResponse(
        "profile.html", context={"request": request, "profile": profile}
    )
    ...
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/danhgiasinhvien"):
                return templates.TemplateResponse(
                    "student_review.html", context={"request": request}
                )
    ...
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/danhsachdetai"):
                return templates.TemplateResponse(
                    "projects.html", context={"request": request}
                )
    ...
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/danhsachkythuctap"):
                return templates.TemplateResponse(
                    "internships.html", context={"request": request}
                )
    ...
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/danhsachnhomthuctap"):
                return templates.TemplateResponse(
                    "groups.html", context={"request": request}
                )
    ...
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/yeucauinphieu"):
                return templates.TemplateResponse(
                    "form_request.html", context={"request": request}
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
    ...
    if token:
        try:
            ...
            if permission == "student":
                return templates.TemplateResponse(
                    "sv_form_request.html", context={"request": request}
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def get_ds_de_tai_profile(id: str):
    return JSONResponse(
        status_code=200, content=get_nhom_thuc_tap_by_user_id_controller(id)
    )
        ...
        try:
            ...
            if username:
                ...
                if result:
                    ...
                    return JSONResponse(
                        status_code=400, content={"status": "User not found"}
                    )
        except jwt.PyJWTError:
            pass
    return RedirectResponse("/login")
...
async def get_all_de_tai(token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "user":
                return JSONResponse(
                    status_code=200, content=get_all_de_tai_thuc_tap_controller()
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
    ...
    if token:
        try:
            ...
            if permission == "user" and check_role(uid, "/danhsachdetai"):
                return JSONResponse(
                    status_code=200, content=get_chi_tiet_de_tai_by_id_controller(id)
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def update_chi_tiet_de_tai_by_id_route(
    id: str, ten: str, mota: str, isDeleted: int, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
...
async def them_de_tai_thuc_tap_route(
    ten: str, mota: str, isDeleted: int, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
...
async def get_all_ky_thuc_tap_route(token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "user":
                return JSONResponse(
                    status_code=200, content=get_all_ky_thuc_tap_controller()
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
    ...
    if token:
        try:
            ...
            if permission == "user":
                return JSONResponse(
                    status_code=200,
                    content=get_ky_thuc_tap_by_username_controller(payload.get("sub")),
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
    ...
    if token:
        try:
            ...
            if permission == "user" and check_role(uid, "/danhsachkythuctap"):
                return JSONResponse(
                    status_code=200,
                    content=get_chi_tiet_ky_thuc_tap_by_id_controller(id),
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def update_chi_tiet_ky_thuc_tap_by_id_route(
    id: str,
    ngaybatdau: str,
    ngayketthuc: str,
    isDeleted: int,
...
async def them_ky_thuc_tap_route(
    ngaybatdau: str,
    ngayketthuc: str,
    isDeleted: int,
    ghichu: str,
...
async def them_cong_viec_nhom_route(
    id: int,
    ngaybatdau: str,
    ngayketthuc: str,
    ten: str,
...
async def get_chi_tiet_nhom_thuc_tap_by_id_route(id: str):
    return JSONResponse(
        status_code=200, content=get_chi_tiet_nhom_thuc_tap_by_id_controller(id)
    )
...
async def get_chi_tiet_nganh_by_id_route(id: str):
    return JSONResponse(
        status_code=200, content=get_chi_tiet_nganh_by_id_controller(id)
    )
...
async def update_chi_tiet_nhom_thuc_tap_by_id_route(
    id: int,
    kytt: int,
    nguoihd: int,
    detai: int,
...
async def them_nhom_thuc_tap_route(
    nguoihd: str,
    kytt: str,
    detai: str,
    soluong: int,
...
async def get_ds_sinh_vien_by_username_route(
    kythuctap: str, nhomthuctap: str, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
            ...
            if permission == "user":
                return JSONResponse(
                    status_code=200,
                    content=get_ds_sinh_vien_by_username_controller(
                        username, kythuctap, nhomthuctap
                    ),
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
...
async def get_dssv_by_kttid_nhomid_username_route(
    kythuctap_id: int, nhomhuongdan_id: int, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
            ...
            if permission == "user":
                return JSONResponse(
                    status_code=200,
                    content=get_dssv_by_kttid_nhomid_username_controller(
                        kythuctap_id, nhomhuongdan_id, username
                    ),
                )
...
async def get_ds_chi_tiet_cong_viec_by_idsinhvien_route(
    sinhvienid: int, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
            ...
            if permission == "user":
                return JSONResponse(
                    status_code=200,
                    content=get_ds_chi_tiet_cong_viec_by_idsinhvien_controller(
                        sinhvienid
                    ),
                )
        except jwt.PyJWKError:
...
async def update_xac_nhan_trang_thai_cong_viec_route(
    idcongviec: int, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
...
async def update_danh_gia_sv_by_id_route(
    sinhvienid: str,
    nhomid: int,
    ythuckyluat_number: float,
    ythuckyluat_text: str,
    ...
    if token:
        try:
            ...
            uid = payload.get("id")
            if permission == "user" and check_role(uid, "/danhgiasinhvien"):
                result = update_danh_gia_sv_by_id_controller(
                    sinhvienid,
                    nhomid,
                    ythuckyluat_number,
                    ythuckyluat_text,
                ...
                if i is not TypeError and i is not None:
                    ...
                    if i["kyhieu_truong"] == "VLUTE":
                        ...
                        if r:
                            ...
                            os.remove(
                                os.path.join(f"DOCX/{username}", f"{i['mssv']}.pdf")
                            )
                            return Response(content=docx_content, headers=headers)
                        else:
                            return JSONResponse(
                                status_code=400, content={"status": "ERR"}
                            )
                    else:
                        return JSONResponse(
                            status_code=200,
                            content={
                                "status": "Phiếu chỉ dành cho sinh viên SPKT Vĩnh Long (VLUTE)"
                            },
                        )
                else:
                    return JSONResponse(
                        status_code=404,
                        content={"status": "Sinh viên chưa có đánh giá"},
                    )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
        ...
        try:
            ...
            if permission == "user":
                ...
                if result:
                    ...
                    return JSONResponse(
                        status_code=400, content={"status": "Badd request"}
                    )
            else:
                raise HTTPException(status_code=404, detail="File not found")
        except jwt.PyJWTError:
                ...
                if i is not TypeError:
                    if i["kyhieu_truong"] == "CTU" or i["kyhieu_truong"] == "DNC":
                        ...
                        r = ctu_xuat_phieu_tiep_nhan(
                            "pdf/phieutiepnhan_ctu.pdf",
                            f"phieutiepnhan_{i['mssv']}.pdf",
                            data,
                            username,
                        )
                        if r:
                        ...
                        return JSONResponse(
                            status_code=200,
                            content={
                                "status": "Phiếu chỉ dành cho SV ĐH Cần Thơ (CTU)"
                            },
                        )
                else:
                    return JSONResponse(status_code=400, content={"status": "ERR"})
            ...
            if permission == "user":
                ...
                if i is not TypeError:
                    if i["kyhieu_truong"] == "CTU" or i["kyhieu_truong"] == "DNC":
                        ...
                        for cv in range(0, 8):
                            try:
                                data[f"tuan{int(cv)+1}_batdau"] = i["congviec"][cv][
                                    "ngaybatdau"
                                ]
                                data[f"tuan{int(cv)+1}_ketthuc"] = i["congviec"][cv][
                                    "ngayketthuc"
                                ]
                                data[f"tuan{int(cv)+1}_congviec"] = i["congviec"][cv][
                                    "tencongviec"
                                ]
                            except IndexError:
                        ...
                        r = ctu_xuat_phieu_giao_viec(
                            "pdf/phieugiaoviec_ctu.pdf",
                            f"phieugiaoviec_{i['sv_mssv']}.pdf",
                            data,
                            username,
                        )
                        if r:
                    ...
                    return JSONResponse(
                        status_code=400,
                        content={"status": "Sinh viên chưa có công việc"},
                    )
            else:
                return JSONResponse(
                    status_code=404, content={"status": "Lỗi khi xuất phiếu"}
                ...
                if i is not TypeError:
                    if i["kyhieu_truong"] == "CTU" or i["kyhieu_truong"] == "DNC":
                        ...
                        r = ctu_xuat_phieu_theo_doi(
                            "pdf/phieutheodoi_ctu.pdf",
                            f"phieutheodoi_{i['sv_mssv']}.pdf",
                            data,
                            username,
                        )
                        if r:
...
async def thong_tin_sinh_vien_route(sv: ThongTinSV):
    result = insert_sinh_vien_controller(
        ...
        sv.nganh,
        sv.khoa,
        sha3_256(bytes(get_gia_tri_tham_so("DEFAULT_PASSWORD"), "utf-8")).hexdigest(),
    )
    if result:
        ...
        if sent:
            insert_taikhoan = insert_taikhoan_sinhvien_controller(
                result,
                sha3_256(
                    bytes(get_gia_tri_tham_so("DEFAULT_PASSWORD"), "utf-8")
                ).hexdigest(),
                1,
            )
        ...
        try:
            ...
            if permission == "user":
                ...
                if result:
                    ...
                    return JSONResponse(
                        status_code=200, content={"status": "BADDDD REQUEST"}
                    )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    else:
...
async def get_chi_tiet_sinh_vien_moi_nhap_thong_tin(id: str):
    return JSONResponse(
        status_code=200, content=get_chi_tiet_sinh_vien_chua_co_nhom_controller(id)
    )
...
async def update_sinh_vien_by_id_route(
    id: int,
    mssv: str,
    hoten: str,
    gioitinh: int,
        ...
        try:
            ...
            if permission == "user":
                ...
                if result:
                    ...
                    return JSONResponse(
                        status_code=400, content={"status": "BADDDD REQUEST"}
                    )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    else:
...
async def gui_mail_otp(email: str):
    try:
        ...
        return JSONResponse(
            status_code=500, content={"status": "Email system has problem"}
        )
...
async def sv_danhgiakythuctap(request: Request, token: str = Cookie(None)):
    if token:
        try:
            ...
            if username:
                return templates.TemplateResponse(
                    "sv_review.html", context={"request": request}
                )
            else:
                return RedirectResponse("/login")
        except jwt.PyJWTError:
...
async def xem_thong_tin_sv_route(username: str):
    return JSONResponse(
        status_code=200, content=xem_thong_tin_sv_controller(email=username)
    )


@app.post("/them_chi_tiet_cong_viec")
async def them_chi_tiet_cong_viec_route(
    data: ChiTietCongViec = Body(...), token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
            ...
            if permission == "user" and check_role(uid, "/giaoviec"):
                ...
                for i in data.sinhvien:
                    ...
                    if result == 1:
                        congviec = (
                            get_chi_tiet_giao_viec_cho_sv_by_id_cong_viec_controller(
                                data.id_congviec, int(i)
                            )
                        )
                        congviec_ghichu = data.ghichu.replace("<br>", "\n")
                        congviec_mota = str(congviec["motacongviec"]).replace(
                            "<br>", "\n"
                        )
                        asyncio.create_task(
                            sendMessageTelegram(
                ...
                return JSONResponse(
                    status_code=200,
                    content={"status": "INSERTED", "result": inserted_result},
                )
            else:
                return JSONResponse(
                    status_code=400, content={"status": "BADDDD REQUEST", "result": {}}
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    else:
...
async def get_chi_tiet_cong_viec_by_id_cong_viec_route(id: int):
    return JSONResponse(
        status_code=200, content=get_chi_tiet_cong_viec_by_id_cong_viec_controller(id)
    )
...
async def get_chi_tiet_cong_viec_by_id_route(id: int):
    return JSONResponse(
        status_code=200, content=get_chi_tiet_cong_viec_by_id_controller(id)
    )
...
async def update_chi_tiet_cong_viec_by_id_route(
    id: int, svid: int, ghichu: str, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
            ...
            uid = payload.get("id")
            if permission == "user" and check_role(uid, "/giaoviec"):
                result = update_chi_tiet_cong_viec_by_id_controller(id, svid, ghichu)
                if result:
                    return JSONResponse(status_code=200, content={"status": "OK"})
                else:
                    return JSONResponse(
...
async def danh_gia_thuc_tap_route(
    email: str,
    id_nhd: int,
    dapan_1: int,
    dapan_2: int,
...
async def danh_gia_nhieu_sv_route(
    dssv: str,
    ythuckyluat_number: float,
    ythuckyluat_text: str,
    tuanthuthoigian_number: float,
...
async def theodoitiendo(request: Request, token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "user" and check_role(uid, "/theodoitiendo"):
                return templates.TemplateResponse(
                    "progress.html", context={"request": request}
                )
...
async def congviecsinhvien(request: Request, token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "student":
                return templates.TemplateResponse(
                    "sv_tasks.html", context={"request": request}
                )
...
async def get_chi_tiet_cong_viec_by_id_cong_viec_email_sv_route(
    id: int, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
    ...
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/quanlytaikhoan"):
                return templates.TemplateResponse(
                    "accounts.html", context={"request": request}
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def update_password_route(
    old_password: str, new_password: str, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
            ...
            username = payload.get("sub")
            if permission == "user":
                result = update_password_controller(
                    username,
                    sha3_256(bytes(old_password, "utf-8")).hexdigest(),
                    sha3_256(bytes(new_password, "utf-8")).hexdigest(),
                )
                if result == 1:
                    return JSONResponse(status_code=200, content={"status": "OK"})
                else:
                    return JSONResponse(
                        status_code=200, content={"status": "NOT_MODIFY"}
                    )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
        ...
        try:
            ...
            if permission == "user" and check_role(uid, "/quanlytaikhoan"):
                ...
                if result == 1:
                    ...
                    return JSONResponse(
                        status_code=200, content={"status": "NOT_BANNED"}
                    )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def update_chi_tiet_tai_khoan_by_id(
    id: int,
    hoten: str,
    sdt: str,
    email: str,
        ...
        try:
            ...
            if permission == "user" and check_role(uid, "/quanlytaikhoan"):
                ...
                if result == 1:
                    ...
                    return JSONResponse(
                        status_code=200, content={"status": "NOT_UPDATE"}
                    )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def them_nguoi_huong_dan(
    hoten: str,
    sdt: str,
    email: str,
    chucdanh: str,
    ...
    if token:
        try:
            ...
            uid = payload.get("id")
            if permission == "user" and check_role(uid, "/quanlytaikhoan"):
                result = them_nguoi_huong_dan_controller(
                    hoten,
                    sdt,
                    email,
                    chucdanh,
                ...
                if isinstance(result, int):
                    ...
                    return JSONResponse(
                        status_code=200, content={"status": "NOT_CREATE"}
                    )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def doi_mat_Khau(request: Request, token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "user":
                return templates.TemplateResponse(
                    "change_password.html", context={"request": request}
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def doi_mat_Khau(request: Request, token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "student":
                return templates.TemplateResponse(
                    "sv_change_password.html", context={"request": request}
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def update_password_sv_route(
    old_password: str, new_password: str, token: str = Cookie(None)
):
    if token:
        try:
            ...
            username = payload.get("sub")
            if permission == "student":
                result = update_password_sv_controller(
                    username,
                    sha3_256(bytes(old_password, "utf-8")).hexdigest(),
                    sha3_256(bytes(new_password, "utf-8")).hexdigest(),
                )
...
async def update_thong_tin_sv_route(
    mssv: str,
    hoten: str,
    gioitinh: int,
    sdt: str,
...
async def danh_muc_nganh(request: Request, token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "user" and check_role(uid, "/danhmucnganh"):
                return templates.TemplateResponse(
                    "danhmucnganh.html", context={"request": request}
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
@app.post("/them_nganh")
async def them_nganh(
    ten: str, kyhieu: str, isDeleted: int, idtruong: int, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        ...
        try:
            ...
            if permission == "user" and check_role(uid, "/danhmucnganh"):
                ...
                if result["status"] == "OK":
                    ...
                    return JSONResponse(
                        status_code=200, content={"status": "NOT_DELETE"}
                    )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def get_ds_loai_yeu_cau_by_sv_route(token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "student":
                return JSONResponse(
                    status_code=200, content=get_ds_loai_yeu_cau_by_sv_controller(sv_id)
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
                ...
                if idloaiyeucau == 3:  # YÊU CẦU IN PHIẾU ĐÁNH GIÁ CÓ ID = 3
                    ...
                    if i is not None and i is not TypeError:
                        ...
                        if result != -1:
                            ...
                            return JSONResponse(
                                status_code=200, content={"status": "NOT OK"}
                            )
                    else:
                        return JSONResponse(
                            status_code=200, content={"status": "INVALID"}
                        )
                else:
                    result = gui_yeu_cau_in_phieu_controller(sv_id, idloaiyeucau)
                    if result != -1:
...
async def gui_yeu_cau_in_phieu_by_nguoi_huong_dan_route(
    list_sv: ListRequest, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
...
async def update_xoa_yeu_cau_in_phieu_by_id_route(
    req: ListRequest, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
...
async def canh_bao_yeu_cau_in_phieu_route(
    loaiyeucau: str, id: int, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
            ...
            if permission == "student":
                ...
                asyncio.create_task(
                    sendMessageTelegram(
                        message=f"<b>Yêu cầu in phiếu mới.</b>\n\n<code><b>Tài khoản:</b> {username}\n"
                        f"<b>Vào lúc: </b>{time}\n"
    ...
    if token:
        try:
            ...
            if permission == "student":
                ...
                if (
                    not isinstance(ttyeucau, dict)
                    or "trangthai" not in ttyeucau
                    or ttyeucau["trangthai"] is None
                    or ttyeucau["trangthai"] is TypeError
                ):
                    return JSONResponse(
                        status_code=200, content={"status": "Phiếu không tồn tại"}
                    )
                if ttyeucau["trangthai"] == 1:
                        ...
                        if i is not TypeError:
                            if (
                                ...
                                if r:
                                    ...
                                    return Response(
                                        content=docx_content, headers=headers
                                    )
                            else:
                                return JSONResponse(
                                    status_code=200,
                    ...
                    # Nếu yêu cầu là in phiếu giao việc
                        ...
                        if i is not TypeError:
                            if (
                                ...
                                for cv in range(0, 8):
                                    try:
                                        data[f"tuan{int(cv)+1}_batdau"] = i["congviec"][
                                            cv
                                        ]["ngaybatdau"]
                                        data[f"tuan{int(cv)+1}_ketthuc"] = i[
                                            "congviec"
                                        ][cv]["ngayketthuc"]
                                        data[f"tuan{int(cv)+1}_congviec"] = i[
                                            "congviec"
                                        ][cv]["tencongviec"]
                                    except IndexError:
                                        data[f"tuan{int(cv)+1}_batdau"] = ""
                    ...
                    # Nếu yêu cầu là in phiếu đánh giá
                        ...
                        return JSONResponse(
                            status_code=400,
                            content={"status": "Chức năng sắp ra mắt :D"},
                        )
                else:
                    return JSONResponse(
                        status_code=200,
                        content={"status": "Phiếu không được phê duyệt"},
                    )
            else:
                return JSONResponse(
                    status_code=404, content={"status": "Lỗi khi xuất phiếu"}
                ...
                if ttyeucau["trangthai"] == 1:
                    ...
                    if ttyeucau["loaiyeucau"] == "In phiếu tiếp nhận":
                        return JSONResponse(
                            status_code=200,
                            content={
                                "status": "Chức năng sắp ra mắt cho sinh viên VLUTE"
                            },
                        )
                        ...
                        if i is not TypeError and i is not None:
                            ...
                            if i["kyhieu_truong"] == "VLUTE":
                                data: dict = {
                                    ...
                                    "r6_number": str(i["khanangnhom_number"]),
                                    "r7_number": str(
                                        i["khananggiaiquyetcongviec_number"]
                                    ),
                                    "r8_number": str(i["danhgiachung_number"]),
                                }
                                r = vlute_xuat_danh_gia(
    ...
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/mappingchucnang"):
                return templates.TemplateResponse(
                    "function_mapping.html", context={"request": request}
                )
...
async def get_all_chuc_nang_route(token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/mappingchucnang"):
                return JSONResponse(
                    status_code=200, content=get_all_chuc_nang_controller()
                )
            else:
                return JSONResponse(
                    status_code=200,
                    content={"status": "Chức năng không dành cho sinh viên"},
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def get_chi_tiet_chuc_nang_by_id_route(id: int, token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "user" and check_role(uid, "/mappingchucnang"):
                return JSONResponse(
                    status_code=200, content=get_chi_tiet_chuc_nang_by_id_controller(id)
                )
            else:
                return JSONResponse(
                    status_code=200,
                    content={"status": "Chức năng này chỉ dành cho quản trị viên"},
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def update_chi_tiet_chuc_nang_by_id_route(
    chucnang: Function, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
    ...
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/danhmucphanquyen"):
                return templates.TemplateResponse(
                    "role.html", context={"request": request}
                )
...
async def get_all_vai_tro_chuc_nang_route(token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/danhmucphanquyen"):
                return JSONResponse(
                    status_code=200, content=get_all_vai_tro_chuc_nang_controller()
                )
            else:
                return JSONResponse(
                    status_code=200,
...
async def get_chi_tiet_vai_tro_route(idvt: int, token: str = Cookie(None)):
    if token:
        try:
            ...
            if permission == "user" and check_role(id, "/danhmucphanquyen"):
                return JSONResponse(
                    status_code=200, content=get_chi_tiet_vai_tro_controller(idvt)
                )
            else:
                return JSONResponse(
                    status_code=200,
...
async def update_vai_tro_route(data: Role_Func, token: str = Cookie(None)):
    if token:
        try:
            ...
            id = payload.get("id")
            if permission == "user" and check_role(id, "/danhmucphanquyen"):
                r = update_vai_tro_controller(data.roleid, data.rolename, data.func)
            return JSONResponse(status_code=200, content={"result": r})
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
async def get_all_vai_tro_route(token: str = Cookie(None)):
    if token:
        try:
            ...
            permission = payload.get("permission")
            id = payload.get("id")
            if permission == "user" and (
                check_role(id, "/danhmucphanquyen") or check_role(id, "/quanlytaikhoan")
            ):
                return JSONResponse(
                    status_code=200, content=get_all_vai_tro_controller()
                )
            else:
                return JSONResponse(
                    status_code=200,
...
@app.post("/update_phan_quyen_nguoi_huong_dan_by_id")
async def update_phan_quyen_nguoi_huong_dan_by_id_route(
    data: UserRole, token: str = Cookie(None)
):
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
            ...
            if permission == "user" and check_role(id, "/quanlytaikhoan"):
                ...
                return JSONResponse(
                    status_code=200, content={"status": "INCORRECT_ROLE"}
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
...
def verify_user_route(credentials: UserCredentials):
    if "@" in credentials.username:
        id = verify_student_controller(
            email=credentials.username,
            password=sha3_256(bytes(credentials.password, "utf-8")).hexdigest(),
        )
        if id:
            return {"isVerified": True, "permission": "student", "id": int(id)}
    else:
        id = verify_user_controller(
    ...
    if token:
        try:
            ...
            if id is not None:
                ...
                return JSONResponse(
                    status_code=200, content={"status": "Không lấy được id từ token"}
                )
        except jwt.PyJWTError:
            return RedirectResponse("/login")
    return RedirectResponse("/login")
